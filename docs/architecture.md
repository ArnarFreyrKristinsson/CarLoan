...
rule set and collects every result, so a caller can show all the reasons a loan was declined at once.

This shows the Open/Closed principle in practice: a new rule is a new class implementing `ILoanRule`. 
Existing rules, the validator and every other lender's rule set stay untouched. The test suite verifies
...
Built with the TDD method, using Osherove naming. Mutation-tested regularly with Stryker to catch
